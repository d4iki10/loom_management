class Admin::OrdersController < ApplicationController
  ...
  def index
    ...
    # 検索の実行（スコープを適用）
    @orders = @orders.search_by_company(params[:company_id])
  ...
  def create
    ...
    # ハッシュの値部分のみを配列として取得
    work_processes = order_params[:work_processes]
    ...
    work_process = WorkProcess.initial_processes_list(start_date)
    # process_estimate_idを入れる
    estimate_work_process = WorkProcess.decide_machine_type(work_process, machine_type_id)
    # 完了見込日時を入れる
    update_work_process = WorkProcess.update_deadline(estimate_work_process, start_date)
    # ５個のハッシュとorderの関連付け
    update_work_process.each do |work_process_data|
  ...
  def validate_machine_selection
    machine_assignments_params = update_order_params[:machine_assignments_attributes]
    # 織機が未指定の場合は特にチェックせずスキップ
    return true unless machine_assignments_params.present?
